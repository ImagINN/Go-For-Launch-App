...
extension RocketView: ViewBuilderProtocol {
    func configuration() {
        buildHierarchy()
        setupStyles()
        setupConstraints()
...
extension RocketView {
    ...
    func configure(with detail: LaunchDetail, imageLoader: ImageLoaderProtocol = ImageLoaderService()) {
        ...
        nameLabel.setStyledText(title: "Name", value: detail.rocketFullName ?? "—")
        descriptionLabel.text = (detail.rocketDescription?.isEmpty == false) ? detail.rocketDescription : "—"
        
        currentImageLoadTask?.cancel()
        imageView.image = nil
        
        if let urlStr = detail.rocketImageURL ?? detail.imageURL,
